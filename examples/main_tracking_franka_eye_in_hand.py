...
class JointVelocityControlInterface(object):
    ...
    def pose_control_go(self, x1):
        ...
        # ------------------------------------------------
        self.actual_xy_executed.append(self.fa.get_pose().translation[0:2].tolist())
        ...
        xy_ee_next = xy_ee_next + 0.0 * (xy_ee_next - xy_ee)
...
MAX_TRANSLATION_DESIRED = 0.2  # the maximum translation when running (dynamic) goto_pose
...
while True:
    ...
    if len(converge_phase_u_list) >= 10:
        ...
        if max_force_recent_applied < 1e-4 or idx > 1200:
            break
        else:
