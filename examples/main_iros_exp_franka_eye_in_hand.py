...
import sliding_pack
#  -------------------------------------------------------------------
from utils.robot_perception import *
#  -------------------------------------------------------------------
...
class SliderConvertor(object):
    def __init__(self, opt) -> None:
        ...
        # initialize panda control interface
        self.panda_control = initialize_tf_and_control(self.beta)
    ...
    def track_path_seg(self, i, x_init):
        ...
        # tag: panda control
        self.panda_control.panda_move_to_contact_face(contact_face_script)
        self.panda_control.panda_start_move()

        # get x_init from perception
        x_init = self.panda_control.panda_get_state_variable(contact_face_script)
        ...
        for idx in range(Nidx-1):
            ...
            # tag: panda control
            x0 = self.panda_control.panda_get_state_variable(contact_face_script)
            x0[2] += theta_offset
            ...
            # tag: panda control
            x1 = (x0 + dyn.f(x0, u0, beta)*dt).elements()
            x1_remove_theta_offset = x1.copy()
            x1_remove_theta_offset[2] -= theta_offset
            self.panda_control.panda_apply_control(x1_remove_theta_offset)
        ...
        # tag: panda control
        rospy.sleep(1)
        self.panda_control.panda_stop_move()
...
solver_opt = {
    ...
    'freq': freq,
    'N_MPC': N_MPC,
    'v_d': 0.02,  # desired tracking velocity [m/s]
