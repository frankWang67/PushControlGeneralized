...
class JointVelocityControlInterface(object):
    ...
    # FIXME: debug this function
    def refind_slider_out_of_view(self):
        ...
        q7_list = []
        for q7 in np.linspace(FC.JOINT_LIMITS_MIN[-1]+0.01, FC.JOINT_LIMITS_MAX[-1]-0.01, 8):
            q7_list.append(q7)
            fa.goto_joints(np.append(fa.get_joints()[0:6], q7), duration=1)
            if check_slider_marker_in_view():
        ...
        if np.array(slider_maker_y_coords).min() >= 1e5:
            print('Warning: cannot find the slider in camera!')
        q7_goal = q7_list[np.argmin(slider_maker_y_coords)]
        fa.goto_joints(np.append(fa.get_joints()[0:6], q7_goal), duration=3)
